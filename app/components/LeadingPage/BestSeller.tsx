...
const formatProductData = (item: ProductApiItem): Product => {
  ...
  const currentPrice =
    typeof currentPriceValue === "number"
      ? currentPriceValue
      : parseFloat(String(currentPriceValue)) || 0;
  ...
  useEffect(() => {
    ...
    const fetchProductsData = async () => {
      ...
      try {
        ...
        const response = await api.get("/product/getBestProduct?best=true");
    ...
    return (
      <section className="bg-white p-4 md:p-8 mx-4 md:mx-8 mt-4 md:mt-8 rounded-lg">
        ...
        <div className="flex justify-center items-center h-40">
          <div className="text-gray-500">No best seller products available</div>
        </div>
      </section>
    );
  }
